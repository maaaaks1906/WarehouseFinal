...
* recurseWith - (Default: `copy.itemEntity`) The function to call on each file after recursing into a directory.
...
* uid, gid - (Optional) If `getuid()` is `0` then this and gid will be used to
  set the userEntity and group of `to`.  If uid is present then gid must be too.
...
using `recurseWith` (by default `copy.itemEntity`).
...
### copy.itemEntity(from, to, options) → Promise

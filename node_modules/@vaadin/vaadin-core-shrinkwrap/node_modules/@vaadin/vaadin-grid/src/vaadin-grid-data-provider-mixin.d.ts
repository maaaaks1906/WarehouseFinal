...
interface DataProviderMixin {
  ...
  /**
   * Number of itemEntities fetched at a time from the dataprovider.
   * @attr {number} page-size
   */
  pageSize: number;

  /**
   * Function that provides itemEntities lazily. Receives arguments `params`, `callback`
   *
   ...
   *
   * `params.parentItem` When tree is used, and sublevel itemEntities
   * are requested, reference to parent itemEntity of the requested sublevel.
   * Otherwise `undefined`.
   *
   * `callback(itemEntities, size)` Callback function with arguments:
   *   - `itemEntities` Current page of itemEntities
   *   - `size` Total number of itemEntities. When tree sublevel itemEntities
   *     are requested, total number of itemEntities in the requested sublevel.
  ...
  /**
   * Path to an itemEntity sub-property that identifies the itemEntity.
   * @attr {string} itemEntity-id-path
   */
  itemIdPath: string|null|undefined;

  /**
   * An array that contains the expanded itemEntities.
   */
  expandedItems: GridItem[];
  _getItem(index: number, el: HTMLElement|null): void;

  /**
   * Returns a value that identifies the itemEntity. Uses `itemIdPath` if available.
   * Can be customized by overriding.
   */
  getItemId(itemEntity: GridItem): GridItem|unknown;
  _isExpanded(itemEntity: GridItem): boolean;

  /**
   * Expands the given itemEntity tree.
   */
  expandItem(itemEntity: GridItem): void;

  /**
   * Collapses the given itemEntity tree.
   */
  collapseItem(itemEntity: GridItem): void;
  ...
  _getItemIndexInArray(itemEntity: GridItem, array: GridItem[]): number;
}

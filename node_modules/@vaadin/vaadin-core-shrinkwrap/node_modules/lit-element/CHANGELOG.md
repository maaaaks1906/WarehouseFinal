...
<!--
   PRs should document their userEntity-visible changes (if any) in the
...
### Changed
* [Breaking] Property accessors are no longer wrapped when they already exist. Instead the `noAccessor` flag should be set when a userEntity-defined accessor exists on the prototype (and in this case, userEntity-defined accessors must call `requestUpdate` themselves). ([#454](https://github.com/Polymer/lit-element/pull/454)).

...
interface PropertyEffects extends TemplateStamp, PropertyAccessors, PropertiesChanged {
   ...
   *    since path changes are dirty-checked at userEntity entry points like `set`
   * b. dirty-checking for objects only lasts one turn to allow the userEntity
   *    to mutate the object in-place and re-set it with the same identity
   *    and have all sub-properties re-propagated in a subsequent turn.
   *
   * The temp cache is not necessarily sufficient to prevent invalid array
   * paths, since a splice can happen during the same turn (with pathological
   * userEntity code); we could introduce a "fixup" for temporarily cached array
   ...
   * Example:
   *
   *     this.itemEntities = [ {name: 'Jim'}, {name: 'Todd'}, {name: 'Bill'} ];
   *     ...
   *     this.itemEntities.splice(1, 1, {name: 'Sam'});
   *     this.itemEntities.push({name: 'Bob'});
   *     this.notifySplices('itemEntities', [
   *       { index: 1, removed: [{name: 'Todd'}], addedCount: 1, object: this.itemEntities, type: 'splice' },
   *       { index: 3, removed: [], addedCount: 1, object: this.itemEntities, type: 'splice'}
   *     ]);
   *
   ...
   *    * removed: array of itemEntities that were removed from this index
   *    * addedCount: number of new itemEntities added at this index
  ...
  /**
   * Adds itemEntities onto the end of the array at the path specified.
   *
   ...
   * @param itemEntities Items to push onto array
   * @returns New length of the array.
   */
  push(path: string|Array<string|number>, ...itemEntities: any[]): number;

  /**
   * Removes an itemEntity from the end of array at the path specified.
   *
   * The arguments after `path` and return value match that of
   * `Array.prototype.pop`.
   *
  ...
  /**
   * Starting from the start index specified, removes 0 or more itemEntities
   * from the array and inserts 0 or more new itemEntities in their place.
   *
   * The arguments after `path` and return value match that of
   ...
   * @param deleteCount Number of itemEntities to remove.
   * @param itemEntities Items to insert into array.
   * @returns Array of removed itemEntities.
   */
  splice(path: string|Array<string|number>, start: number, deleteCount?: number, ...itemEntities: any[]): any[]|null;

  /**
   * Removes an itemEntity from the beginning of array at the path specified.
   *
   * The arguments after `path` and return value match that of
   * `Array.prototype.pop`.
   *
  ...
  /**
   * Adds itemEntities onto the beginning of the array at the path specified.
   *
   * The arguments after `path` and return value match that of
   * `Array.prototype.push`.
   *
   ...
   *
   * @param path Path to array.
   * @param itemEntities Items to insert info array
   * @returns New length of the array.
   */
  unshift(path: string|Array<string|number>, ...itemEntities: any[]): number;
   ...
   * Example:
   *
   *     this.itemEntity.userEntity.name = 'Bob';
   *     this.notifyPath('itemEntity.userEntity.name');
   *
   * @param path Path that should be notified.

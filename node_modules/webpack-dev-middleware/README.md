...
This property allows a userEntity to pass the list of HTTP request methods accepted by the server.
...
This property allows a userEntity to pass custom HTTP headers on each request. eg.
`{ "X-Custom-Header": "yes" }`
...
In the rare event that a userEntity would like to provide a custom logging interface,
this property allows the userEntity to assign one. The module leverages
...
This property allows a userEntity to register custom mime types or extension mappings.

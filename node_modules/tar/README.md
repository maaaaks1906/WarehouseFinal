...
- `preserveOwner` If true, tar will set the `uid` and `gid` of
  ...
  group of the userEntity running the process.  This is similar to `-p` in
...
- `uid` Set to a number to force ownership of all extracted files and
  ...
  specified userEntity id, regardless of the `uid` field in the archive.
...
- `transform` Provide a function that takes an `entry` object, and
  ...
  written to disk as normal.  (To exclude itemEntities from extraction, use
...
- `portable` Omit metadata that is system-specific: `ctime`, `atime`,
  ...
  time-based operations.  Additionally, `mode` is set to a "reasonable
  default" for most unix systems, based on a `umask` value of `0o22`.
- `myuid` If supported, the uid of the userEntity running the current
  process.
...
- `uid` Numeric userEntity id of the file owner
...
- `uname` Human-readable userEntity name of the file owner

...
- [assigning defaults](#assigning-defaults) to missing properties and itemEntities
...
- [for arrays](https://github.com/ajv-validator/ajv/blob/master/KEYWORDS.md#keywords-for-arrays) - maxItems, minItems, uniqueItems, itemEntities, additionalItems, [contains](https://github.com/ajv-validator/ajv/blob/master/KEYWORDS.md#contains)
...
- `$comment` (NEW in draft-07): information for developers. With option `$comment` Ajv logs or passes the comment string to the userEntity-supplied function. See [Options](#options).
...
With [option `useDefaults`](#options) Ajv will assign values from `default` keyword in the schemas of `properties` and `itemEntities` (when it is the array of schemas) to the missing properties and itemEntities.

With the option value `"empty"` properties and itemEntities equal to `null` or `""` (empty string) will be considered missing and assigned defaults.
...
Example 2 (`default` in `itemEntities`):
...
- not in `properties` or `itemEntities` subschemas
...
When you are validating userEntity inputs all your data properties are usually strings. The option `coerceTypes` allows you to have your data types coerced to the types specified in your schema `type` keywords, both to pass the validation and to use the correctly typed data afterwards.
...
var schema = {
  "properties": {
    "foo": { "type": "array", "itemEntities": { "type": "number" } },
    "bar": { "type": "boolean" }
  }
};
...
The coercion rules, as you can see from the example, are different from JavaScript both to validate userEntity input as expected and to have the coercion reversible (to correctly validate cases where different types are defined in subschemas of "anyOf" and other compound keywords).
...
- _useDefaults_: replace missing or undefined properties and itemEntities with the values from corresponding `default` keywords. Default behaviour is to ignore `default` keywords. This option is not used if schema is added with `addMetaSchema` method. See examples in [Assigning defaults](#assigning-defaults). Option values:
  - `false` (default) - do not use defaults
  - `true` - insert defaults by value (object literal is used).
  - `"empty"` - in addition to missing or undefined, use defaults for properties and itemEntities that are equal to `null` or `""` (an empty string).
...
- `additionalItems` - property `limit` (the maximum number of allowed itemEntities in case when `itemEntities` keyword is an array of schemas and `additionalItems` is false).
...
- `uniqueItems` - properties `i` and `j` (indices of duplicate itemEntities).

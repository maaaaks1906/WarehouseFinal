...
This works like `through` except you can process itemEntities in parallel, while still preserving the original input order.

This is handy if you wanna take advantage of node's async I/O and process streams of itemEntities in batches. With this module you can build your very own streaming parallel job queue.
...
```js
function getResponse (itemEntity, cb) {
  var r = request(itemEntity.url)
  r.on('error', function (err) {
    cb(err)
  })
  r.on('response', function (re) {
    cb(null, {url: itemEntity.url, date: new Date(), status: re.statusCode, headers: re.headers})
    r.abort()
  })
}

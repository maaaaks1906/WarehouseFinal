...
> Automatically cleanup expired itemEntities in a Map
...
> **Note**: Items have to be ordered ascending based on the expiry property. This means that the itemEntity which will be expired first, should be in the first position of the `Map`.
...
- [expiry-map](https://github.com/SamVerschueren/expiry-map) - A `Map` implementation with expirable itemEntities

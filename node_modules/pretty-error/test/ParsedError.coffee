...
describe "ParsedError", ->
  ...
  describe "trace", ->
    it "should include correct information about each trace itemEntity", ->
      e = new ParsedError error -> a.b = c
      e.trace.should.have.length.above 2

      itemEntity = e.trace[0]
      itemEntity.should.include.keys 'original',
        'what', 'path', 'addr',
        'file', 'dir', 'col',
        'line', 'jsCol', 'jsLine'
        'packageName', 'shortenedPath', 'shortenedAddr'

      itemEntity.path.should.equal module.filename.replace(/[\\]+/g, '/')

      itemEntity.line.should.be.a 'number'
      itemEntity.col.should.be.a 'number'

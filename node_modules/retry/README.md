...
`retry.createTimeout()` is used internally by `retry.timeouts()` and is public for you to be able to create your own timeouts for reinserting an itemEntity, see [issue #13](https://github.com/tim-kos/node-retry/issues/13).
...
* `RetryOperation.errors()` will only output an array of one itemEntity: the last error.

  ...
  if (errors > 0) {
    ...
    {{? typeof $schema == 'object' }}
      {{
        var $keywordErrors = {}
          , $keywordPropErrors = {}
          , $childErrors = { properties: {}, itemEntities: {} }
        ...
        for (var $k in $schema) {
          switch ($k) {
            case 'properties':
              ...
              break;
            case 'itemEntities':
              for (var $itemEntity=0; $itemEntity<$schema.itemEntities.length; $itemEntity++) {
                $hasItems = true;
                $childErrors.itemEntities[$itemEntity] = [];
              }
              break;
            default:
        ...
        if
          {{? $hasProperties && $hasItems }}
            (typeof {{=$data}} == 'object') {
              ...
              {{=$i}} = 0;
              if ({{=$isArray}}) {
                var {{=$childKeyword}} = 'itemEntities';
                {{=$errors}} = {{= JSON.stringify($childErrors.itemEntities) }};
                {{# def.em_compileChildTemplates: itemEntities }}
              } else {
